...
class HomeController extends Controller
{
    ...
    public function getHomeVideos2(Request $request){
        $result = array('status'=>'');
        $limit = $request->input('limit', 10);
        $uid = $request->input('uid');
        $image = DB::raw('concat("'.env('MEDIA_URL_IMAGE').'/",image_location) as image');
    ...
    public function getHomeDocuments(Request $request){
        ...
        try {
            ...
            $documents = Documents::where('is_home', true)->select('id', 'title', $image,
            'documents.chef', 'time_to_done', 'documents.content',
                'level', $days,  'view_count', 'is_home')
                ->orderby('date_created', 'desc')->take($limit)->get();
