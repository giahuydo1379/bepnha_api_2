...
class BachKhoaController extends Controller
{
    ...
    private function getQueryDocuments($uid) {
        ...
        $liked = DB::raw('notebook_document.document_id IS NOT NULL as liked');
        $query = DB::table('documents')
            ->select('documents.id', 'documents.title', 'documents.content', $image, 'documents.chef', 'documents.time_to_done',
                'documents.level',  $days,  'documents.view_count', $liked)
            ->where('documents.disable', 0)
            ->leftJoin('notebook_document', 'documents.id', '=', 'notebook_document.document_id', 'and', 'notebook_document.user_id', '=', $uid)->distinct();
        return $query;
    }
}